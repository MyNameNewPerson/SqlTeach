    ...
    lessons: [
      {
        ...
        content: [
          ...
          'Ячейка - одно конкретное значение на пересечении строки и колонки. Например, status = paid у заказа 1004.',
          'В следующем модуле ты узнаешь, как соединять таблицы запросом. Пока достаточно понимать, что связи существуют и строятся по id.',
        ],
...
FROM customers;`,
      ...
      },
      {
        ...
        minutes: 10,
        content: [
          'PRIMARY KEY - это главный уникальный номер строки. В таблице customers каждый клиент получает свой id: 1, 2, 3, 4, 5. Никакие два клиента не могут иметь одинаковый id.',
          'Зачем это нужно: имя клиента может измениться, город может измениться, но id = 5 всегда будет означать именно этого клиента. Все связи в базе строятся по id, а не по имени, поэтому данные не ломаются при изменениях.',
          "Пример: если ты ищешь клиента по имени 'Delta Shop' и компания переименовалась в 'Delta Market', поиск по имени сломается. Но WHERE customer_id = 5 найдёт его всегда.",
          'FOREIGN KEY - это колонка в одной таблице, которая содержит id из другой таблицы. В orders колонка customer_id со значением 5 означает: этот заказ принадлежит клиенту с id = 5 из customers.',
          'Это и есть связь: не по имени, а по числу. Так ERP понимает, какой клиент сделал какой заказ, даже если клиентов тысячи.',
          'Частая ошибка: если в orders.customer_id написано 999, а клиента с id = 999 в customers нет, это висячая ссылка. Такие записи называют orphan records, и они вызывают баги в ERP.',
        ],
        sql: `SELECT id, customer_id, status
FROM orders
WHERE customer_id = 5;`,
        engineerNote:
          'На собеседовании важно объяснить не только термин, но и риск: без ключей ERP легко получит заказ, который указывает в никуда.',
      },
    ],
    quiz: [
...
FROM orders
WHERE id = 1004;`,
      'Сначала найди сам заказ в orders. Потом словами объясни, что дальше нужно проверить payments и integration_logs, но JOIN появится в следующих модулях.',
    ),
  },
  {
    id: 'module-1',
...
ORDER BY order_date DESC;`,
  ...
  },
  {
    ...
    lessons: [
      {
        title: 'Короткие имена таблиц - зачем и как',
        minutes: 8,
        content: [
          'Когда в запросе несколько таблиц, удобно давать им короткие имена. Это называется алиас.',
          'FROM orders o - здесь o это псевдоним таблицы orders. FROM customers c - c это псевдоним таблицы customers.',
          'После этого вместо orders.id можно писать o.id, а вместо customers.name - c.name.',
          'Зачем: когда в запросе три таблицы, писать полные имена везде неудобно. Алиасы делают код короче и читаемее.',
          'Правило: алиас задаётся один раз после имени таблицы в строке FROM или JOIN, и потом используется везде в этом запросе.',
        ],
        sql: `-- Без алиасов:
...
-- С алиасами (то же самое, короче):
SELECT o.id, c.name
FROM orders o
LEFT JOIN customers c ON c.id = o.customer_id;`,
        engineerNote:
          'На интервью алиасы не нужно произносить как магию: это просто короткие имена таблиц внутри одного запроса.',
      },
      {
