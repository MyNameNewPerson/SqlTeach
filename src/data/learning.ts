...
export const memoryMethod = [
  {
    title: '1. ЧИТАЙ',
    text: 'Прочитай объяснение и посмотри на таблицу с данными. Не пытайся запомнить всё - сначала пойми общую идею.',
  },
  {
    title: '2. ЗАПУСКАЙ',
    text: 'Скопируй SQL из примера в песочницу и запусти. Потом измени одно условие, например id = 1004 на id = 1005, и запусти снова.',
  },
  {
    title: '3. ОБЪЯСНЯЙ',
    text: 'Скажи вслух одно предложение: “Этот запрос ищет [что], потому что [зачем]”.',
  },
  {
    title: '4. РЕШАЙ КЕЙС',
    text: 'Выполни мини-задание в конце модуля. Сначала попробуй сам, потом смотри подсказку.',
  },
]
...
export const workedExamples: Record<string, WorkedExample> = {
  'module-0': {
    title: 'Первый запрос: посмотреть клиентов',
    scenario: 'Нужно увидеть, как SQL выбирает колонки из одной таблицы. Связи и JOIN будут позже.',
    sql: `SELECT id, name, city
FROM customers;`,
    lineByLine: [
      'SELECT - говоришь базе, какие колонки тебя интересуют. Как выбрать столбцы в Excel, которые хочешь видеть.',
      'id, name, city - три колонки, которые появятся в результате.',
      'FROM - говоришь, в какой таблице искать. Пока у нас одна таблица - customers.',
      'В следующих модулях будет несколько таблиц, но сначала важно уверенно читать одну.',
    ],
    whyThisWay: 'Первый шаг не должен перегружать: ученик видит таблицу, строки, колонки и результат запроса без новых терминов.',
    commonMistake: 'Сразу прыгать в JOIN и алиасы, не поняв, что SELECT выбирает колонки, а FROM выбирает таблицу.',
    tryNext: 'Замени city на active и посмотри, как изменится набор колонок.',
  },
  'module-1': {
...
ORDER BY order_date DESC;`,
    lineByLine: [
      'WHERE - это фильтр. Без него ты видишь все строки сразу.',
      'AND - оба условия должны быть верны одновременно: статус подходит и дата попадает в февраль.',
      "OR - хотя бы одно из условий должно быть верно. Для статусов его часто заменяют на IN.",
      "IN - короткая замена для нескольких OR: IN ('new', 'paid') вместо status = 'new' OR status = 'paid'.",
      "BETWEEN - диапазон включительно: BETWEEN '2026-02-01' AND '2026-02-28'.",
    ],
...
ROLLBACK;`,
  ...
  'module-7': {
    ...
    scenario: 'Нужно найти строки, которые ссылаются на несуществующий заказ.',
    sql: `SELECT p.id, p.order_id, p.status
FROM payments p
LEFT JOIN orders o ON o.id = p.order_id
WHERE o.id IS NULL;`,
    lineByLine: [
      'LEFT JOIN - оставляет все строки из левой таблицы, даже если в правой таблице нет совпадения.',
      'Это главный JOIN для поиска проблем в данных: платеж не пропадёт, даже если заказ справа не найден.',
      'WHERE ... IS NULL после LEFT JOIN находит строки без совпадения.',
      "В результате видны 'висячие' записи: payment.order_id указывает туда, где нет orders.id.",
    ],
    whyThisWay: 'Для диагностики плохих связей нужен LEFT JOIN, потому что INNER JOIN скрыл бы проблемную строку.',
    commonMistake: 'Проверять только p.order_id IS NULL и пропустить случай, где order_id заполнен, но такого заказа нет.',
    tryNext: 'Сделай такой же запрос для inventory -> products.',
  },
  'module-8': {
